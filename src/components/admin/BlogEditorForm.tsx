...
import type { BlogCategory, BlogPostRecord, LocaleText, SeoMeta } from "@/types/content";
...
export function BlogEditorForm({ initial }: Props) {
  ...
  const [readTime, setReadTime] = useState<LocaleText>(
    initial?.readTime ?? { tr: "5 dk", en: "5 min" }
  );
  const [slug, setSlug] = useState(initial?.slug ?? "");
  const [category, setCategory] = useState<BlogCategory>(initial?.category ?? "Technology");
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-6">
      ...
      <div className="grid gap-5 rounded-3xl border border-white/10 bg-white/[0.03] p-6">
        ...
        />
        <RichTextEditor
          key={`blog-content-${locale}`}
          label={`İçerik (${locale.toUpperCase()})`}
          value={content[locale]}
          onChange={(html) => setContent((s) => ({ ...s, [locale]: html }))}
        />
        <AdminField
          label={`Okuma süresi (${locale.toUpperCase()})`}
          value={readTime[locale]}
          onChange={(v) => setReadTime((s) => ({ ...s, [locale]: v }))}
        />
      </div>

      <div className="grid gap-5 rounded-3xl border border-white/10 bg-white/[0.03] p-6 md:grid-cols-2">
        <AdminField label="Slug" value={slug} onChange={setSlug} placeholder="ornek-yazi-slug" />
        <label className="block">
          <span className="text-xs uppercase tracking-[0.2em] text-white/45">Kategori</span>
          <select
